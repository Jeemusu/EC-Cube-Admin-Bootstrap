...
$(function() {
    ...
    for (var i = 0; i < product_class_id.length; i++) {
        ...
        for (var j = 0; j < $(quantity[i]).val(); j++) {
            ...
            // 数量
            var qfield = $('<input type="text" class="input-mini" name="multiple_quantity[' + index + ']" size="4" value="1" />');
...
<!--▼検索フォーム-->
<form style="padding:0 20px;margin:0;" name="form1" id="form1" method="post" action="<!--{$smarty.server.REQUEST_URI|h}-->">
...
<div class="btn-area">
   <a class="btn btn-primary" href="javascript:;" onclick="func_submit(); return false;"><span class="btn-next">決定</span></a>
</div>
</form>
