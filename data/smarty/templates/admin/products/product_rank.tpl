...
<div id="products" class="contents-main">
    ...
    <!--{* ▼画面左 *}-->
    <div id="products-rank-left" class="pull-left">
    ...
    <!--▼画面右-->
    <div id="products-rank-right" class="pull-right">
        <h2><!--{* jQuery で挿入される *}--></h2>
            ...
            <table class="table" id="categoryTable">
                ...
                <!--{section name=cnt loop=$arrProductsList}-->
                    <tr>
                        ...
                        <p><div class="input-append"><input type="text" name="pos-<!--{$arrProductsList[cnt].product_id}-->" class="input-mini" />番目へ<a class=" btn add-on" href="?" onclick="fnModeSubmit('move','product_id', '<!--{$arrProductsList[cnt].product_id}-->'); return false;">移動</a></div></p>
                        <!--{/if}-->
                        <!--{if !($smarty.section.cnt.first && $tpl_disppage eq 1)}-->
                        <a class="btn" href="?" onclick="fnModeSubmit('up','product_id', '<!--{$arrProductsList[cnt].product_id}-->'); return false;"><i class="icon-chevron-sign-up"></i></a>
                        <!--{/if}-->
                        <!--{if !($smarty.section.cnt.last && $tpl_disppage eq $tpl_pagemax)}-->
                        <a class="btn" href="?" onclick="fnModeSubmit('down','product_id', '<!--{$arrProductsList[cnt].product_id}-->'); return false;"><i class="icon-chevron-sign-down"></i></a>
                        <!--{/if}-->
                        </td>
                    </tr>
                <!--{/section}-->
