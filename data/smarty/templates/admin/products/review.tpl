...
<input type="hidden" name="mode" value="search" />
    ...
    <table class="table">
        <tr>
            <th>投稿者名</th>
            <td><input type="text" name="search_reviewer_name" value="<!--{$arrForm.search_reviewer_name|h}-->" class="span3" /></td>
            <th>投稿者URL</th>
            <td><input type="text" name="search_reviewer_url" value="<!--{$arrForm.search_reviewer_url|h}-->" class="span3" /></td>
        </tr>
        <tr>
            <th>商品名</th>
            <td><input type="text" name="search_name" value="<!--{$arrForm.search_name|h}-->" class="span3" /></td>
            <th>商品コード</th>
            <td><input type="text" name="search_product_code" value="<!--{$arrForm.search_product_code|h}-->" class="span3" /></td>
        </tr>
        <tr>
            <th>性別</th>
            <!--{assign var=key value=search_sex}-->
            <td><div class="inline"><!--{html_checkboxes name="$key" options=$arrSex selected=$arrForm[$key]}--></div></td>
            <th>おすすめレベル</th>
            <td>
...
<form name="form1" id="form1" method="post" action="?">
    ...
    <div class="btn-area-top">
        ...
        <!--{if $smarty.const.ADMIN_MODE == '1'}-->
            <a class="btn" href="javascript:;" onclick="fnModeSubmit('delete_all','',''); return false;"><span>検索結果をすべて削除</span></a>
        <!--{/if}-->
        <a class="btn" href="javascript:;" onclick="fnModeSubmit('csv','',''); return false;"><span><i class="icon-download"></i> CSV ダウンロード</span></a>
    </div>
    <!--{if $arrReview > 0 & $tpl_linemax > 0}-->
        ...
        <!--検索結果表示テーブル-->
        <table id="products-review-result" class="table">
            <tr>
                <th>投稿日</th>
                <th>投稿者名</th>
                <th>商品名</th>
                <th>おすすめレベル</th>
                <th class="center">表示・非表示</th>
            ...
            <!--{section name=cnt loop=$arrReview}-->
                <tr>
                    ...
                    <td class="center"><!--{if $arrReview[cnt].status eq 1}-->表示<!--{elseif $arrReview[cnt].status eq 2}-->非表示<!--{/if}--></td>
                    <td class="center"><a class="btn" href="javascript:;" onclick="fnChangeAction('./review_edit.php'); fnModeSubmit('','review_id','<!--{$arrReview[cnt].review_id}-->'); return false;">編集</a></td>
                    <td class="center"><a class="btn btn-danger" href="javascript:;" onclick="fnModeSubmit('delete','review_id','<!--{$arrReview[cnt].review_id}-->'); return false;">削除</a></td>
                </tr>
            <!--{/section}-->
        </table>
    <!--{/if}-->
