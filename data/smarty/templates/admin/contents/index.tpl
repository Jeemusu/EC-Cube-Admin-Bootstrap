...
<input type="hidden" name="term" value="" />
    ...
    <table class="table">
        <tr>
            ...
            <td>
                ...
                <select class="input-small" name="year" <!--{if $arrErr.year || $arrErr.month || $arrErr.day }-->style="background-color:<!--{$smarty.const.ERR_COLOR|h}-->"<!--{/if}-->>
                    <option value="" selected="selected">----</option>
                    <!--{html_options options=$arrYear selected=$arrForm.year}-->
                </select> 年
                <select class="input-mini" name="month" <!--{if $arrErr.year || $arrErr.month || $arrErr.day}-->style="background-color:<!--{$smarty.const.ERR_COLOR|h}-->"<!--{/if}-->>
                    <option value="" selected="selected">--</option>
                    <!--{html_options options=$arrMonth selected=$arrForm.month}-->
                </select> 月
                <select class="input-mini" name="day" <!--{if $arrErr.year || $arrErr.month || $arrErr.day}-->style="background-color:<!--{$smarty.const.ERR_COLOR|h}-->"<!--{/if}-->>
                    <option value="" selected="selected">--</option>
        ...
        <tr>
            ...
            <td>
                ...
                <textarea name="news_title" rows="8" class="span6" maxlength="<!--{$smarty.const.MTEXT_LEN}-->" <!--{if $arrErr.news_title}-->style="background-color:<!--{$smarty.const.ERR_COLOR|h}-->"<!--{/if}-->><!--{"\n"}--><!--{$arrForm.news_title|h}--></textarea><br />
        ...
        <tr>
            ...
            <td>
                ...
                <input type="text" name="news_url" class="span5"    value="<!--{$arrForm.news_url|h}-->" <!--{if $arrErr.news_url}-->style="background-color:<!--{$smarty.const.ERR_COLOR|h}-->"<!--{/if}--> maxlength="<!--{$smarty.const.URL_LEN}-->" />
        ...
        <tr>
            ...
            <td>
                ...
                <textarea name="news_comment" rows="8" wrap="soft" class="span6" maxlength="<!--{$smarty.const.LTEXT_LEN}-->" style="background-color:<!--{if $arrErr.news_comment}--><!--{$smarty.const.ERR_COLOR|h}--><!--{/if}-->"><!--{"\n"}--><!--{$arrForm.news_comment|h}--></textarea><br />
                <span class="warning"> (上限3000文字)</span>
            </td>
        </tr>
    </table>
    <!--{* ▲登録テーブルここまで *}-->
    ...
    <div class="btn-area">
       	<a class="btn" href="<!--{$smarty.const.ROOT_URLPATH}--><!--{$smarty.const.ADMIN_DIR}-->contents/"><span class="btn-next">キャンセル</span></a>
        <a class="btn btn-primary" href="javascript:;" onclick="return func_regist();"><span class="btn-next">この内容で登録する</span></a>
    </div>
</form>
    ...
    <table class="table">
        ...
        <tr style="background:<!--{if $arrNews[data].news_id != $tpl_news_id}-->#ffffff<!--{else}--><!--{$smarty.const.SELECT_RGB}--><!--{/if}-->;" class="center">
            ...
            <td class="center">
                ...
                <a href="#" class="btn" onclick="return func_edit('<!--{$arrNews[data].news_id|h}-->');">編集</a>
                <!--{else}-->
                編集中
                <!--{/if}-->
            </td>
            <td class="center"><a class="btn btn-danger" href="#" onclick="return func_del('<!--{$arrNews[data].news_id|h}-->');">削除</a></td>
            <td class="center">
            <!--{if count($arrNews) != 1}-->
            <p><div class="input-append"><input type="text" name="pos-<!--{$arrNews[data].news_id|h}-->" size="2" class="input-mini" /> <a href="?" onclick="fnFormModeSubmit('move', 'moveRankSet','news_id', '<!--{$arrNews[data].news_id|h}-->'); return false;" class="btn">番目へ移動</a></div></p>
            <!--{/if}-->
            <!--{if $arrNews[data].rank ne $max_rank}--><a class="btn" href="#" onclick="return func_rankMove('up', '<!--{$arrNews[data].news_id|h}-->', '<!--{$max_rank|h}-->');"><i class="icon-chevron-sign-up"></i></a><!--{/if}-->　<!--{if $arrNews[data].rank ne 1}--><a class="btn" href="#" onclick="return func_rankMove('down', '<!--{$arrNews[data].news_id|h}-->', '<!--{$max_rank|h}-->');"><i class="icon-chevron-sign-down"></i></a><!--{/if}-->
            </td>
        </tr>
        <!--{sectionelse}-->
        <tr class="center">
