...
<div id="admin-contents">
    <table class="table" id="recommend-table">
        ...
        <tr>
            <td>おすすめ商品(<!--{$smarty.section.cnt.iteration}-->)</td>
                                ...
                                <div class="detail-name">商品名： <!--{$arrItems[$smarty.section.cnt.iteration].name|h}--></div>
                                    <div class="detail-form">
                                        <form name="form<!--{$smarty.section.cnt.iteration}-->" id="form<!--{$smarty.section.cnt.iteration}-->" method="post" action="?">
                                            ...
                                            <textarea class="top" name="comment" cols="45" rows="4" style="width: 450px; height: 80px; <!--{$arrErr[$smarty.section.cnt.iteration].comment|sfGetErrorColor}-->" <!--{$arrItems[$smarty.section.cnt.iteration].product_id|sfGetEnabled}-->><!--{"\n"}--><!--{$arrItems[$smarty.section.cnt.iteration].comment|h}--></textarea>
                                        </form>
                                    </div>
                                </div>
                            </div>
                ...
                <!--{else}-->
                    <td class="AlignLeft">
                        <a class="btn" href="javascript:;" onclick="lfnCheckSetItem('<!--{$smarty.section.cnt.iteration}-->'); return false;" target="_blank"><span class="btn-next">商品を選択する</span></a>
                        <form name="form<!--{$smarty.section.cnt.iteration}-->" id="form<!--{$smarty.section.cnt.iteration}-->" method="post" action="?">
                            <input type="hidden" name="<!--{$smarty.const.TRANSACTION_ID_NAME}-->" value="<!--{$transactionid}-->" />
                            <input type="hidden" name="mode" value="regist" />
                            <input type="hidden" name="product_id" value="<!--{$arrItems[$smarty.section.cnt.iteration].product_id|h}-->" />
            ...
            <td class="center">
                <!--{if $arrItems[$smarty.section.cnt.iteration].product_id}-->
                    <a class="btn" href="javascript:;" onclick="lfnCheckSetItem('<!--{$smarty.section.cnt.iteration}-->'); return false;" target="_blank">
                        編集</a>
                <!--{else}-->
                    - -
                <!--{/if}-->
            ...
            <td class="center">
                ...
                <!--{if $smarty.section.cnt.iteration != 1 && $arrItems[$smarty.section.cnt.iteration].product_id}-->
                    <p></p><a class="btn" href="?" onclick="lfnSortItem('up',<!--{$arrItems[$smarty.section.cnt.iteration].rank}-->); return false;"><i class="icon-chevron-sign-up"></i></a></p>
                <!--{/if}-->
                <!--{if $smarty.section.cnt.iteration < $tpl_disp_max_rank && $arrItems[$smarty.section.cnt.iteration].product_id}-->
                    <a class="btn" href="?" onclick="lfnSortItem('down',<!--{$arrItems[$smarty.section.cnt.iteration].rank}-->); return false;"><i class="icon-chevron-sign-down"></i></a>
                <!--{/if}-->
            </td>
        </tr>
    ...
    <div class="btn-area">
        ...
        <!--{if $arrItems[$smarty.section.cnt.iteration].product_id}-->
                        <a class="btn btn-danger" href="javascript:;" onClick="return fnInsertValAndSubmit( document.form<!--{$smarty.section.cnt.iteration}-->, 'mode', 'delete', '削除します。宜しいですか' )">削除</a>

                <!--{/if}-->
        
        <a class="btn-primary btn" href="javascript:;" onclick="lfnCheckSubmit(document.form<!--{$smarty.section.cnt.iteration}-->); return false;"><span class="btn-next">この内容で登録する</span></a>
           <!--{/if}-->
    </div>
