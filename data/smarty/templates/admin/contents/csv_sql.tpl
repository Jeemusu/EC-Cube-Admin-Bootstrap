...
<div id="admin-contents" class="contents-main">
    ...
    <!--{if $arrSqlList}-->
        <table id="contents-csv-sqllist" class="table">
            <!--{foreach key=key item=item from=$arrSqlList}-->
                <tr style="background-color:<!--{if $item.sql_id == $arrForm.sql_id}--><!--{$smarty.const.SELECT_RGB}--><!--{else}-->#ffffff<!--{/if}-->;">
                    ...
                    <td>
                        <div>
                            <a class="btn" href="javascript:;" name='csv' onclick="fnTargetSelf(); fnFormModeSubmit('form1','csv_output','csv_output_id',<!--{$item.sql_id}-->); return false;"><span>CSV出力</span></a>
                            <a class="btn btn-danger" href="javascript:;" name='del' onclick="fnTargetSelf(); fnFormModeSubmit('form1','delete','sql_id',<!--{$item.sql_id}-->); return false;"><span>削除</span></a>
                        </div>
                    </td>
                </tr>
            <!--{/foreach}-->
    ...
    <div class="btn-area-top">
         <a class="btn" href="javascript:;" name='subm' onclick="fnTargetSelf(); fnFormModeSubmit('form1','new_page','',''); return false;"><span>SQLを新規入力</span></a>
         <a class="btn" href="javascript:;" name="subm" onclick="doPreview(); return false;"><span>構文エラーを確認する</span></a>
    </div>


    <h2>SQL設定<!--{if $arrForm.sql_id != ""}-->(編集中:<!--{$arrForm.sql_name|h}-->)<!--{else}-->(新規入力)<!--{/if}--></h2>
    <table id="contents-csv-sqlset" class="table">
        <tr>
        ...
        <tr>
            ...
            <td align="left">
                ...
                <div>
                    <textarea style="width:600px;" name="csv_sql" cols=50 rows=10 align="left" wrap=off style="<!--{if $arrErr.csv_sql != ""}-->background-color: <!--{$smarty.const.ERR_COLOR}-->;<!--{/if}-->"><!--{"\n"}--><!--{$arrForm.csv_sql|h}--></textarea>
                </div>
            </td>
        </tr>
    </table>
    ...
    <div class="btn-area">
        <a class="btn btn-primary" href="javascript:;" name="subm" onclick="fnTargetSelf(); fnFormModeSubmit('form1', 'confirm', '', '')"><span class="btn-next">この内容で登録する</span></a>
    </div>
