        ...
        <table class="table">
            <tr>
                ...
                <td>
                    ...
                    <input type="file" name="upload_file" size="40" <!--{if $arrErr.upload_file}-->style="background-color:<!--{$smarty.const.ERR_COLOR|h}-->"<!--{/if}-->><a class="btn" href="javascript:;" onclick="setTreeStatus('tree_status');fnModeSubmit('upload','',''); return false;"><i class="icon-upload"></i> アップロード</a>
                </td>
            </tr>
            <tr>
                <th>フォルダ作成</th>
                <td>
                    ...
                    <input type="text" name="create_file" value="" style="width:336px;<!--{if $arrErr.create_file}--> background-color:<!--{$smarty.const.ERR_COLOR|h}--><!--{/if}-->"> <a class="btn" href="javascript:;" onclick="setTreeStatus('tree_status');fnModeSubmit('create','',''); return false;">作成</a>
                </td>
            </tr>
        </table>
        <h2><!--{* jQuery で挿入される *}--></h2>
        <table class="table">
            ...
            <!--{section name=cnt loop=$arrFileList}-->
                ...
                <tr id="<!--{$id}-->" style="">
                    ...
                    <!--{if $arrFileList[cnt].is_dir}-->
                        <td class="center">
                            <a class="btn" href="javascript:;" onclick="fnSetFormVal('form1', 'tree_select_file', '<!--{$arrFileList[cnt].file_path}-->');fnSelectFile('<!--{$id}-->', '#808080');fnModeSubmit('move','',''); return false;">表示</a>
                        </td>
                    <!--{else}-->
                        <td class="center">
                            <a class="btn" href="javascript:;" onclick="fnSetFormVal('form1', 'select_file', '<!--{$arrFileList[cnt].file_path|h}-->');fnSelectFile('<!--{$id}-->', '#808080');fnModeSubmit('view','',''); return false;">表示</a>
                        </td>
                    <!--{/if}-->
                    <!--{if $arrFileList[cnt].is_dir}-->
                        <!--{* ディレクトリはダウンロード不可 *}-->
                    ...
                    <!--{else}-->
                        <td class="center">
                            <a class="btn" href="javascript:;" onclick="fnSetFormVal('form1', 'select_file', '<!--{$arrFileList[cnt].file_path|h}-->');fnSelectFile('<!--{$id}-->', '#808080');setTreeStatus('tree_status');fnModeSubmit('download','',''); return false;">ダウンロード</a>
                        </td>
                    <!--{/if}-->
                    <td class="center">
                        <a class="btn btn-danger" href="javascript:;" onclick="fnSetFormVal('form1', 'select_file', '<!--{$arrFileList[cnt].file_path|h}-->');fnSelectFile('<!--{$id}-->', '#808080');setTreeStatus('tree_status');fnModeSubmit('delete','',''); return false;">削除</a>
                    </td>
                </tr>
            <!--{/section}-->
        </table>
